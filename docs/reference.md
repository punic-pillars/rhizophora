...
│   │   # Creates Server with 13 tools. Handles ListToolsRequestSchema
...
│   │  ── Tools Layer ──────────────────────────────────────
│   │
│   ├── tools/                           # Tool handlers (one per MCP tool, 14 files)
...
**Purpose**: All 13 MCP tool handlers follow a uniform return type and error handling pattern.
...
└── tool-schemas.ts       # TOOL_INPUT_SCHEMAS for all 13 tools
```
...
2. Define `TOOL_DEFINITIONS` array — 13 tools with name, description, and inputSchema
